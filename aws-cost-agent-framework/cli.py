...
Examples:
  ...
  %(prog)s --region us-west-2 --output custom_report.html
    ...
    aws_group.add_argument('--profile', default='4bfast',
                          help='AWS profile name (default: 4bfast)')
    ...
    # Language and AI Configuration
    lang_group = parser.add_argument_group('Language and AI Configuration')
    lang_group.add_argument('--language', choices=['pt-BR', 'en-US'], default='pt-BR',
                           help='Report language (default: pt-BR)')
    lang_group.add_argument('--ai-profile', default='4bfast',
                           help='AWS profile for Bedrock AI access (default: 4bfast)')
    lang_group.add_argument('--no-ai', action='store_true',
                           help='Disable AI-powered summary and recommendations')
    lang_group.add_argument('--no-ai-summary', action='store_true',
                           help='Disable AI summary (keep recommendations)')
    lang_group.add_argument('--no-ai-recommendations', action='store_true',
                           help='Disable AI recommendations (keep summary)')
...
def create_settings_from_args(args) -> Settings:
    ...
    settings.ai.enabled = not args.no_ai
    settings.ai.aws_profile = args.ai_profile
    settings.ai.include_summary = not args.no_ai_summary and not args.no_ai
    settings.ai.include_recommendations = not args.no_ai_recommendations and not args.no_ai
...
async def main():
    ...
    try:
        ...
        if not args.quiet:
            ...
            print(f"   Language: {settings.report.language}")
            print(f"   AI Summary: {'Enabled' if settings.ai.enabled and settings.ai.include_summary else 'Disabled'}")
            print(f"   AI Recommendations: {'Enabled' if settings.ai.enabled and settings.ai.include_recommendations else 'Disabled'}")
            if settings.ai.enabled:
                print(f"   AI Profile: {settings.ai.aws_profile}")
            print()
