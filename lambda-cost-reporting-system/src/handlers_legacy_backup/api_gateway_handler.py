...
Updated: 2025-11-03 - Fixed CORS configuration
"""
...
CORS_ALLOWED_ORIGINS = os.environ.get('CORS_ALLOWED_ORIGINS', 'https://costhub.4bfast.com.br').split(',')[0]
...
class APIGatewayHandler:
    ...
    async def handle_accounts_endpoints(self, method: str, path: str, params: Dict, body: str, user_context: Dict) -> Dict[str, Any]:
        ...
        elif method == 'POST' and path == '/api/v1/accounts':
            return await self.create_account(body, user_context)
        else:
    ...
    async def create_account(self, body: str, user_context: Dict) -> Dict[str, Any]:
        """Create new account/provider"""
        try:
            if not body:
                return self.error_response('Request body is required', 400)
            ...
            # Add user context
            account_data['created_by'] = user_context.get('sub', 'unknown')
            account_data['created_at'] = datetime.utcnow().isoformat()
            
            # Call backend to create account
            backend_data = await self.call_backend_post('/accounts', account_data)
            
            if backend_data:
                return self.success_response(backend_data, 201)
            else:
                return self.error_response('Failed to create account', 500)
    ...
    async def call_backend_post(self, endpoint: str, data: Dict) -> Optional[Dict]:
        """Call existing backend API with POST"""
        try:
            async with aiohttp.ClientSession() as session:
                url = f"{self.backend_url}{endpoint}"
            ...
            print(f"Backend POST call failed: {str(e)}")
            return None
