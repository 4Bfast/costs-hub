...
NO MORE HARDCODING - Uses real JWT client_id extraction
"""
import json
import boto3
import uuid
...
from utils.jwt_utils import extract_client_id_from_token, validate_cognito_token, get_client_id_for_user

# Initialize DynamoDB client using config
...
def validate_authorization_inline(event, cors_headers):
    """Inline authorization validation - USES REAL JWT EXTRACTION"""
    ...
    try:
        ...
        user_info = validate_cognito_token(access_token)
        ...
        return False, {
            'statusCode': 401,
            'headers': cors_headers,
            'body': json.dumps({'message': f'Unauthorized: {str(e)}'})
        }, None

def get_client_id_for_user(username):
    """
    DEPRECATED: This function is replaced by extract_client_id_from_token()
    Kept for backward compatibility but should not be used for new code.
    """
    # Use the new JWT extraction method instead
