...
class FrontendAPIStack(Stack):
    ...
    def _create_api_handler_function(self) -> _lambda.Function:
        ...
        function.add_to_role_policy(
            iam.PolicyStatement(
                ...
                resources=[
                    f"arn:aws:dynamodb:{self.region}:{self.account}:table/costhub-accounts"
                ]
            )
        )
    ...
    def _create_api_gateway(self) -> apigateway.RestApi:
        ...
        # Add auth resource without authorization
        auth_resource = api_v1.add_resource("auth")
        auth_resource.add_method("ANY", apigateway.LambdaIntegration(self.api_handler_function))
        auth_resource.add_resource("{proxy+}").add_method("ANY", apigateway.LambdaIntegration(self.api_handler_function))
        
        # Add proxy resource for all other routes (with authorization)
