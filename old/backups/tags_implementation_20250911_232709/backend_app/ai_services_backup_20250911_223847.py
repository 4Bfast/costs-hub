...
Responda APENAS com a explicação, sem saudações ou despedidas."""
    ...
    def _build_trends_prompt(self, trends_data: Dict) -> str:
        ...
        prompt_template = """You are 'Costs-Hub AI', a FinOps expert. Analyze the following cost variation data between two periods.

### COST DATA ###
{dados_json}
### END OF DATA ###

Generate an executive summary with EXACTLY 3 short topics (maximum 15 words each). Use this format:

• [Topic 1 about general trend]
• [Topic 2 about main service OR new services if any]  
• [Topic 3 with specific recommendation]

FOCUS ON:
1. General trend (healthy growth vs inefficiency)
2. Main service responsible for variation OR highlight new services if they exist
3. Specific recommended action

PRIORITY: If there are new services ("newServices" field), ALWAYS mention in topic 2.

Use dollar values ($X.XX). Respond in Portuguese (Brazil) with ONLY the 3 topics, no introduction."""

        return prompt_template.format(dados_json=dados_json)

    def _generate_fallback_trends_analysis(self, trends_data: Dict) -> str:
        """
        Gera análise básica de tendências quando IA não está disponível
        """
        try:
            total_change = trends_data.get('totalCostChange', 0)
            top_movers = trends_data.get('topMovers', [])
            new_services = trends_data.get('newServices', [])
            
            if total_change > 0:
                trend_desc = f"aumento de ${total_change:.2f}"
                impact = "crescimento" if total_change < 1000 else "expansão significativa"
            elif total_change < 0:
                trend_desc = f"redução de ${abs(total_change):.2f}"
                impact = "otimização"
            else:
                trend_desc = "estabilidade"
                impact = "controle efetivo"
            
            analysis = f"• Período apresentou {trend_desc}, indicando {impact} da infraestrutura\n"
            
            # Priorizar novos serviços se existirem
            if new_services:
                services_list = ", ".join(new_services[:2])  # Máximo 2 serviços
                analysis += f"• Novos serviços detectados: {services_list}\n"
            elif top_movers:
                top_service = top_movers[0]
                service_name = top_service.get('service', 'N/A')
                service_change = top_service.get('variationValue', 0)
                
                if service_change > 0:
                    analysis += f"• {service_name} lidera com aumento de ${service_change:.2f}\n"
                else:
                    analysis += f"• {service_name} apresentou redução de ${abs(service_change):.2f}\n"
            else:
                analysis += "• Nenhuma variação significativa detectada\n"
            
            # Recomendação baseada no contexto
            if new_services:
                analysis += "• Monitore novos serviços para evitar custos inesperados"
            elif len(top_movers) > 1:
                analysis += f"• Analise os {len(top_movers)} principais serviços para otimização"
            else:
                analysis += "• Configure AWS Bedrock para insights mais avançados"
            
            return analysis + " [Análise local - Configure AWS Bedrock para insights mais avançados]"
            
        except Exception as e:
            logger.error(f"Erro ao gerar análise de tendências fallback: {e}")
            return "Não foi possível gerar análise de tendências no momento. Os dados detalhados estão disponíveis nas tabelas abaixo."
