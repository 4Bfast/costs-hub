...
# Importar novos serviços de IA e cache
from .ai_services import generate_dashboard_summary, get_term_explanation, generate_trends_analysis
from .cache_service import ai_summary_cache, generate_cache_key, ai_explanation_cache, generate_explanation_cache_key
...
def get_dashboard_data_internal(current_user, start_date_str, end_date_str):
    """
    Função auxiliar para obter dados do dashboard sem depender de request.args
    """
    try:
        start_date = datetime.strptime(start_date_str, '%Y-%m-%d').date()
        end_date = datetime.strptime(end_date_str, '%Y-%m-%d').date()
    ...
    # Obter contas-membro da organização
    member_accounts = MemberAccount.query.filter_by(organization_id=current_user.organization_id).all()
    ...
    for cost in all_costs:
        cost_data = {
            'member_account_id': cost.member_account_id,
            'aws_service': cost.aws_service,
            'cost': float(cost.cost or 0),
    ...
    total_current = sum(c['cost'] for c in current_period_costs)
    total_previous = sum(c['cost'] for c in previous_period_costs)
    variation_value = total_current - total_previous
    variation_percentage = (variation_value / total_previous * 100) if total_previous > 0 else 0
    
    # Agrupar por serviço para variações
    current_by_service = defaultdict(float)
    previous_by_service = defaultdict(float)
    
    for cost in current_period_costs:
        current_by_service[cost['aws_service']] += cost['cost']
    
    for cost in previous_period_costs:
        previous_by_service[cost['aws_service']] += cost['cost']
    
    # Calcular variações por serviço
    service_variations = []
    all_services = set(current_by_service.keys()) | set(previous_by_service.keys())
    ...
    for service in all_services:
        current_cost = current_by_service.get(service, 0)
        previous_cost = previous_by_service.get(service, 0)
        service_variation = current_cost - previous_cost
        
        if previous_cost > 0:
            service_variation_pct = (service_variation / previous_cost) * 100
        else:
            service_variation_pct = 100.0 if current_cost > 0 else 0.0
        
        service_variations.append({
            'service': service,
            'currentCost': current_cost,
            'previousCost': previous_cost,
            'variationValue': service_variation,
            'variationPercentage': service_variation_pct
        })
    
    # Ordenar por valor absoluto da variação
    service_variations.sort(key=lambda x: abs(x['variationValue']), reverse=True)
    
    return {
        'kpis': {
            'totalCost': total_current,
            'totalVariationValue': variation_value,
            'totalVariationPercentage': variation_percentage,
            'taxCost': 0,
            'credits': 0
        },
        'serviceVariation': service_variations[:10],  # Top 10
        'costByAccount': [],  # Simplificado para IA
        'chartData': {}  # Não necessário para IA
    }

@api_bp.route('/dashboards/ai-summary', methods=['GET'])
@token_required
def get_ai_dashboard_summary(current_user):
    """
    Endpoint que gera resumo executivo dos dados do dashboard usando IA
    """
    # 1. Validação dos parâmetros
    start_date_str = request.args.get('start_date')
    end_date_str = request.args.get('end_date')
    ...
    try:
        ...
        if cached_summary:
            logging.info(f"Resumo IA retornado do cache para org {current_user.organization_id}")
            return jsonify({
                'summary': cached_summary,
                'cached': True
            })
        
        # 3. Obter dados do dashboard usando função auxiliar
        try:
            dashboard_data = get_dashboard_data_internal(current_user, start_date_str, end_date_str)
            if not dashboard_data:
                return jsonify({'error': 'Erro ao obter dados do dashboard'}), 500
        except Exception as e:
            logger.error(f"Erro ao obter dados do dashboard para IA: {e}")
            return jsonify({'error': 'Erro ao obter dados do dashboard'}), 500
        
        # 4. Gerar resumo com IA
        logging.info(f"Gerando resumo IA para org {current_user.organization_id}")
        summary_text = generate_dashboard_summary(dashboard_data)
        
        # 5. Salvar no cache (TTL de 6 horas)
        ai_summary_cache.set(cache_key, summary_text, ttl_seconds=21600)
        ...
        logging.error(f"Erro ao gerar resumo IA: {e}")
        return jsonify({
            'error': 'Erro interno ao gerar resumo',
            'details': str(e) if hasattr(current_user, 'is_admin') and current_user.is_admin() else None
        }), 500

@api_bp.route('/ai/explain', methods=['POST'])
@token_required
def explain_term(current_user):
    """
    Endpoint que explica termos técnicos AWS usando IA
    """
    try:
        # 1. Validar corpo da requisição
        data = request.get_json()
        if not data or 'term' not in data or 'context' not in data:
            return jsonify({'error': 'Campos "term" e "context" são obrigatórios'}), 400
        
        term = data['term'].strip()
        context = data['context'].strip()
        
        if not term or not context:
            return jsonify({'error': 'Campos "term" e "context" não podem estar vazios'}), 400
        
        # 2. Verificar cache primeiro (TTL de 30 dias)
        cache_key = generate_explanation_cache_key(term, context)
        cached_explanation = ai_explanation_cache.get(cache_key)
        
        if cached_explanation:
            logging.info(f"Explicação retornada do cache para termo: {term}")
            return jsonify({
                'explanation': cached_explanation,
                'cached': True
            })
        
        # 3. Gerar explicação com IA
        logging.info(f"Gerando explicação IA para termo: {term}")
        explanation_text = get_term_explanation(term, context)
        
        # 4. Salvar no cache (TTL de 30 dias = 2592000 segundos)
        ai_explanation_cache.set(cache_key, explanation_text, ttl_seconds=2592000)
        ...
        logging.error(f"Erro ao explicar termo: {e}")
        return jsonify({
            'error': 'Erro interno ao gerar explicação',
            'details': str(e) if hasattr(current_user, 'is_admin') and current_user.is_admin() else None
        }), 500

@api_bp.route('/trends/analysis', methods=['GET'])
@token_required
def get_trends_analysis(current_user):
    """
    Endpoint simplificado para análise de tendências
    """
    try:
        start_date_str = request.args.get('start_date')
        end_date_str = request.args.get('end_date')
        ...
        # Obter dados básicos usando função auxiliar existente
        dashboard_data = get_dashboard_data_internal(current_user, start_date_str, end_date_str)
        if not dashboard_data:
        ...
        return jsonify({
            'success': True,
            'trendTables': {
                'byService': dashboard_data['serviceVariation'][:10],
        ...
        return jsonify({'error': 'Erro interno na análise de tendências'}), 500

@api_bp.route('/trends/ai-summary', methods=['GET'])
@token_required
def get_trends_ai_summary(current_user):
    """
    Endpoint separado para resumo IA
    """
    try:
        start_date_str = request.args.get('start_date')
        end_date_str = request.args.get('end_date')
        ...
        trends_data = {
            'totalCostChange': dashboard_data['kpis']['totalVariationValue'],
            'totalCostChangePercent': dashboard_data['kpis']['totalVariationPercentage'],
            'topMovers': dashboard_data['serviceVariation'][:5]
        }
        
        # Adicionar informação sobre novos serviços
        try:
            # Buscar novos serviços dos últimos 7 dias
            from datetime import datetime, timedelta
            end_date = datetime.now().date()
            ...
            # Serviços dos últimos 3 dias
            recent_services = set()
            recent_costs = DailyFocusCosts.query.filter(
                DailyFocusCosts.member_account_id.in_(member_account_ids),
                DailyFocusCosts.usage_date >= end_date - timedelta(days=3)
            ).all()
            
            for cost in recent_costs:
                recent_services.add(cost.aws_service)
            
            # Serviços dos 4 dias anteriores
            older_services = set()
            older_costs = DailyFocusCosts.query.filter(
                DailyFocusCosts.member_account_id.in_(member_account_ids),
                DailyFocusCosts.usage_date >= start_date,
                DailyFocusCosts.usage_date < end_date - timedelta(days=3)
            ).all()
            
            for cost in older_costs:
                older_services.add(cost.aws_service)
            
            # Novos serviços = presentes nos últimos 3 dias mas não nos 4 anteriores
            new_services = list(recent_services - older_services)
            trends_data['newServices'] = new_services[:3]  # Top 3 para IA
            
        except Exception as e:
            logging.warning(f"Erro ao detectar novos serviços: {e}")
            trends_data['newServices'] = []
        
        # Gerar análise com IA
        ai_summary = generate_trends_analysis(trends_data)
        ...
        logging.error(f"Erro no resumo IA: {e}")
        return jsonify({
            'aiSummary': 'Resumo IA temporariamente indisponível. Os dados detalhados estão disponíveis abaixo.',
            'success': False
        })

@api_bp.route('/trends/chart', methods=['GET'])
@token_required
def get_trends_chart(current_user):
    """
    Endpoint separado para dados do gráfico mensal
    """
    try:
        from datetime import datetime, timedelta
        from collections import defaultdict
        ...
        member_accounts = MemberAccount.query.filter_by(organization_id=current_user.organization_id).all()
        if not member_accounts:
            return jsonify({'error': 'Nenhuma conta AWS encontrada'}), 404
        
        member_account_ids = [acc.id for acc in member_accounts]
        
        # Últimos 6 meses de dados
        end_date = datetime.now().date()
        start_date = end_date.replace(day=1) - timedelta(days=180)  # ~6 meses
        
        # Buscar dados mensais
        monthly_data = DailyFocusCosts.query.filter(
            DailyFocusCosts.member_account_id.in_(member_account_ids),
            DailyFocusCosts.usage_date >= start_date,
            DailyFocusCosts.usage_date <= end_date
        ).all()
        ...
        for cost in monthly_data:
            month_key = cost.usage_date.strftime('%Y-%m')
            monthly_costs[month_key] += float(cost.cost or 0)
        ...
        while current_date <= end_date:
            ...
            chart_values.append(monthly_costs.get(month_key, 0))
        ...
        logging.error(f"Erro no gráfico: {e}")
        return jsonify({
            'chartData': {
...
@api_bp.route('/trends/forecast', methods=['GET'])
@token_required
def get_trends_forecast(current_user):
    """
    Endpoint para previsão baseada nos últimos 7 dias
    """
    try:
        from datetime import datetime, timedelta
        from collections import defaultdict
        ...
        for cost in recent_costs:
            day_key = cost.usage_date.strftime('%Y-%m-%d')
            cost_value = float(cost.cost or 0)
            
            daily_totals[day_key] += cost_value
            service_trends[cost.aws_service].append({
                'date': day_key,
                'cost': cost_value
            })
        
        # Calcular tendência diária (últimos 7 dias)
        daily_values = [daily_totals.get((start_date + timedelta(days=i)).strftime('%Y-%m-%d'), 0) 
                       for i in range(7)]
        
        # Média dos últimos 3 dias vs primeiros 4 dias
        recent_avg = sum(daily_values[-3:]) / 3 if len(daily_values) >= 3 else 0
        older_avg = sum(daily_values[:4]) / 4 if len(daily_values) >= 4 else 0
        
        # Taxa de crescimento diário - limitada para valores pequenos
        if older_avg > 0 and recent_avg > 0:
            raw_growth_rate = (recent_avg - older_avg) / older_avg
            # Limitar taxa de crescimento para valores realistas (máximo 20% ao dia)
            daily_growth_rate = max(-0.2, min(0.2, raw_growth_rate))
        else:
            daily_growth_rate = 0
        
        # Projeção para 30 dias - mais conservadora
        current_daily_avg = recent_avg
        if current_daily_avg > 0:
            projected_30_days = current_daily_avg * 30 * (1 + daily_growth_rate)
        else:
            projected_30_days = 0
        
        # Detectar novos serviços (serviços que apareceram nos últimos 3 dias)
        new_services = []
        for service, costs in service_trends.items():
            recent_costs_service = [c['cost'] for c in costs if c['date'] in [
                (end_date - timedelta(days=i)).strftime('%Y-%m-%d') for i in range(3)
            ]]
            older_costs_service = [c['cost'] for c in costs if c['date'] in [
                (end_date - timedelta(days=i)).strftime('%Y-%m-%d') for i in range(4, 7)
            ]]
            
            if sum(recent_costs_service) > 0 and sum(older_costs_service) == 0:
                new_services.append({
                    'service': service,
                    'dailyCost': sum(recent_costs_service) / len(recent_costs_service)
                })
        
        return jsonify({
            'dailyAverage': current_daily_avg,
            'growthRate': daily_growth_rate * 100,  # Em percentual
            'projected30Days': projected_30_days,
            'projectedChange': projected_30_days - (current_daily_avg * 30),
            'newServices': new_services[:5],  # Top 5 novos serviços
            'dailyTrend': daily_values,
            'success': True
        })
...
@api_bp.route('/trends/projects-forecast', methods=['GET'])
@token_required
def get_projects_forecast(current_user):
    """
    Endpoint para projeção de custos por projeto baseada nos últimos 7 dias
    """
    try:
        from datetime import datetime, timedelta
        from collections import defaultdict
        ...
        # Obter contas-membro da organização
        member_accounts = get_member_accounts(current_user)
        ...
        recent_costs = DailyFocusCosts.query.filter(
            DailyFocusCosts.account_id.in_(member_accounts),
            DailyFocusCosts.usage_date >= start_date,
            DailyFocusCosts.usage_date <= end_date
        ).all()
        
        # Agrupar por projeto usando tags
        ...
        for cost in recent_costs:
            ...
            # Determinar projeto baseado em tags (prioridade)
            project_name = None
            
            # Prioridade 1: Tag 'Project' ou 'projeto'
            if hasattr(cost, 'resource_tags') and cost.resource_tags:
                tags = cost.resource_tags if isinstance(cost.resource_tags, dict) else {}
                project_name = tags.get('Project') or tags.get('projeto') or tags.get('PROJECT')
            
            # Prioridade 2: Environment + Application
            if not project_name and hasattr(cost, 'resource_tags') and cost.resource_tags:
                env = tags.get('Environment') or tags.get('environment') or tags.get('ENV')
                app = tags.get('Application') or tags.get('application') or tags.get('APP')
                if env and app:
                    project_name = f"{app}-{env}"
            
            # Fallback: Nome da conta AWS
            if not project_name:
                project_name = f"Conta-{cost.account_id}"
            
            # Agrupar dados
            projects_data[project_name]['daily_costs'][day_key] += cost_value
            projects_data[project_name]['total_cost'] += cost_value
            projects_data[project_name]['account_name'] = cost.account_id
            projects_data[project_name]['services'].add(cost.aws_service)
        
        # Calcular projeções para cada projeto
        projects_forecast = []
        
        for project_name, data in projects_data.items():
            daily_values = [data['daily_costs'].get((start_date + timedelta(days=i)).strftime('%Y-%m-%d'), 0) 
                           for i in range(7)]
            ...
            if older_avg > 0 and recent_avg > 0:
                raw_growth_rate = (recent_avg - older_avg) / older_avg
                growth_rate = max(-0.3, min(0.5, raw_growth_rate))  # Limite -30% a +50%
            else:
                growth_rate = 0
            
            # Projeção 30 dias
            current_daily_avg = recent_avg if recent_avg > 0 else data['total_cost'] / 7
            projected_30_days = current_daily_avg * 30 * (1 + growth_rate)
            projected_change = projected_30_days - (current_daily_avg * 30)
            
            # Status do projeto
            variation_percent = (projected_change / (current_daily_avg * 30)) * 100 if current_daily_avg > 0 else 0
            
            if variation_percent > 50 or projected_change > 100:
                status = 'alert'  # 🔴
            elif variation_percent > 30 or projected_change > 50:
                status = 'warning'  # 🟡
            else:
                status = 'normal'  # 🟢
            ...
            projects_forecast.append({
                ...
                'status': status,
                'servicesCount': len(data['services'])
            })
        
        # Ordenar por custo atual (maior primeiro)
        projects_forecast.sort(key=lambda x: x['currentCost'], reverse=True)
        
        return jsonify({
            'projects': projects_forecast,
            'totalProjects': len(projects_forecast),
            'success': True
        })
        
    except Exception as e:
        logging.error(f"Erro na projeção por projetos: {e}")
        return jsonify({
            'projects': [],
            'totalProjects': 0,
            'success': False,
