...
async function loadTrendsFocusedAI() {
  ...
  try {
    await new Promise(resolve => setTimeout(resolve, 2000));
    trendsFocusedAI.value = `• Tendência de crescimento de 15% nos últimos 6 meses indica expansão saudável
• EC2 e RDS lideram crescimento, sugerindo aumento de workloads de produção
• Novos serviços Lambda detectados indicam migração para arquitetura serverless`;
  } catch (err) {
...
async function loadForecastData() {
  ...
  try {
    ...
    forecastData.value = {
      ...
      growthRate: 2.5,
      newServices: [
        { service: 'Lambda', dailyCost: 5.2 },
        { service: 'API Gateway', dailyCost: 2.1 }
      ]
    };
  } finally {
    ...
    <Message v-else severity="info" :closable="false" class="mb-4">
      ...
      <TabPanel header="Tendências">
        <div class="trends-content tw-space-y-6">
          ...
          <!-- Resumo IA Focado em Tendências -->
          <Card class="ai-summary-card">
            <template #header>
              <div class="tw-p-4">
                <h3 class="tw-text-xl tw-font-semibold tw-text-white tw-flex tw-items-center tw-gap-2">
            ...
            <template #content>
              <div v-if="isLoadingTrendsFocusedAI" class="tw-flex tw-items-center tw-gap-2 tw-text-white">
                <ProgressSpinner size="small" />
                <span>Analisando tendências de crescimento...</span>
              </div>
              <p v-else class="tw-text-white tw-m-0 tw-whitespace-pre-line tw-text-lg">{{ trendsFocusedAI }}</p>
            </template>
          </Card>
          ...
          <Card class="forecast-card">
            ...
            <template #content>
              ...
              <div v-else class="tw-space-y-4 tw-text-white">
                <div class="tw-flex tw-justify-between">
                  <span>Custo Projetado:</span>
                  <span class="tw-font-bold">${{ getForecastValue().toFixed(2) }}</span>
                </div>
                <div class="tw-flex tw-justify-between">
                  <span>Variação Esperada:</span>
                  <span class="tw-font-bold">${{ getForecastChange().toFixed(2) }} ({{ getForecastPercent().toFixed(1) }}%)</span>
                </div>
                <div class="tw-flex tw-justify-between" v-if="forecastData.growthRate">
                  <span>Taxa de Crescimento Diário:</span>
                  <span class="tw-font-bold">{{ forecastData.growthRate.toFixed(2) }}%</span>
                </div>
                <div v-if="forecastData.newServices?.length > 0" class="tw-mt-4">
                  <strong class="tw-block tw-mb-2">Novos Serviços Detectados:</strong>
                  <ul class="tw-list-disc tw-list-inside tw-space-y-1">
                    <li v-for="service in forecastData.newServices" :key="service.service" class="tw-text-sm">
          ...
          <!-- Tabela de Projeção por Serviço -->
          <Card v-if="trendsData?.trendTables?.byService">
            <template #title>Projeção por Serviço - Próximos 30 Dias</template>
            <template #content>
              <DataTable 
                :value="trendsData.trendTables.byService" 
                :paginator="true" 
                :rows="10"
              ...
              >
                <Column field="service" header="Serviço" :sortable="true">
                  <template #body="slotProps">
                    <strong>{{ slotProps.data.service }}</strong>
                  </template>
                </Column>
                
                <Column field="currentCost" header="Custo Atual" :sortable="true" class="tw-text-right">
                  <template #body="slotProps">
                ...
                <Column header="Projeção 30 Dias" :sortable="false" class="tw-text-right">
                  <template #body="slotProps">
                    <div class="tw-flex tw-items-center tw-justify-end tw-gap-2">
                      <ProgressBar 
                        ...
                        :max="100"
                        severity="info"
                        class="tw-w-16 tw-h-2"
                        :showValue="false"
                      />
                      <span class="tw-font-semibold tw-text-blue-600">
                        ${{ ((slotProps.data.currentCost || 0) * 1.15).toFixed(2) }}
                      </span>
                    </div>
                  </template>
                </Column>
                
                <Column header="Tendência" :sortable="false" class="tw-text-center">
                  <template #body="slotProps">
                    <span :class="(slotProps.data.variationValue || 0) >= 0 ? 'tw-text-orange-600 tw-font-semibold' : 'tw-text-green-600 tw-font-semibold'">
                      {{ (slotProps.data.variationValue || 0) >= 0 ? '↗ Crescimento' : '↘ Redução' }}
                    </span>
                  </template>
                </Column>
              </DataTable>
