...
from .security import enhanced_token_required, admin_required, rate_limit, jwt_auth
import boto3
...
@rate_limit(limit=10, window=900, per='ip')  # 10 login attempts per 15 minutes per IP
def login():
    """
    Endpoint para autenticar um usuário e retornar tokens JWT.
    """
    data = request.get_json()
    ...
    try:
        ...
        logging.info(f"User {user.email} logged in successfully from IP {request.remote_addr}")
        
        return jsonify(tokens), 200
        
    except Exception as e:
        logging.error(f"Error generating tokens for user {user.email}: {e}")
        return jsonify({'error': 'Authentication failed'}), 500

@auth_bp.route('/refresh', methods=['POST'])
@rate_limit(limit=50, window=3600, per='ip')  # 50 refresh attempts per hour per IP
def refresh_token():
    """
    Endpoint para renovar tokens JWT usando refresh token.
    """
    data = request.get_json()
    ...
    try:
        ...
        claims = jwt_auth.validate_token(refresh_token, 'refresh')
        user = claims['user']
        
        # Generate new tokens
        tokens = jwt_auth.generate_tokens(user)
        
        logging.info(f"Tokens refreshed for user {user.email}")
...
@rate_limit(limit=10, window=3600, per='user')  # 10 AWS account additions per hour
@enhanced_token_required()
...
@rate_limit(limit=200, window=3600, per='user')
@enhanced_token_required()
